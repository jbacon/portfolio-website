...
function GetInnerText(reactNode: React.ReactNode & React.ReactNode[] | React.ReactNode | React.ReactNode[]): string {
    if (!reactNode) {
        return ""
    }
    ...
    if (React.isValidElement(reactNode)) {
        ...
        const props = reactElement.props as { children?: React.ReactNode & React.ReactNode[]}
        if (props.children) {
